...
class ListUserAdapter(private val listUser: List<ResponseDataUserItem>) : RecyclerView.Adapter<ListUserAdapter.ListViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ListViewHolder, position: Int) {
        holder.binding.txtUsernameList.text = listUser[position].login
        holder.binding.txtNameList.text = listUser[position].reposUrl
        Glide.with(holder.itemView.context).load(listUser[position].avatarUrl).into(holder.binding.imgUserList)
        holder.binding.cardUserList.setOnClickListener{
            val moveDetail = Intent(holder.itemView.context, DetailActivity::class.java)
            moveDetail.putExtra(MainActivity.ARG_USERNAME, listUser[position].login)
